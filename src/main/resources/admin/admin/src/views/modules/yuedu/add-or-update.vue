    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="wuziId" name="wuziId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="月度用料数量" prop="yueduShuliang">
                       <el-input v-model="ruleForm.yueduShuliang"
                                 placeholder="月度用料数量" clearable  :readonly="ro.yueduShuliang"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="月度用料数量" prop="yueduShuliang">
                           <el-input v-model="ruleForm.yueduShuliang"
                                     placeholder="月度用料数量" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info' && !ro.yueduContent"  label="月度用料介绍" prop="yueduContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.yueduContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.yueduContent" label="月度用料介绍" prop="yueduContent">
                            <span v-html="ruleForm.yueduContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="月度用料类型" prop="yueduTypes">
                        <el-select v-model="ruleForm.yueduTypes" :disabled="ro.yueduTypes" placeholder="请选择月度用料类型">
                            <el-option
                                v-for="(item,index) in yueduTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="月度用料类型" prop="yueduValue">
                        <el-input v-model="ruleForm.yueduValue"
                            placeholder="月度用料类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                   ...
                   yueduShuliang: [
                              { required: true, message: '月度用料数量不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   yueduContent: [
                              { required: true, message: '月度用料介绍不能为空', trigger: 'blur' },
                          ],
                   yueduDelete: [
                   ...
                   yueduTypes: [
                              { required: true, message: '月度用料类型不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=yuedu_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.yueduTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                ...
                _this.$http({
                    ...
                    if (data && data.code === 0) {
                        ...
                        _this.ruleForm.yueduContent = _this.ruleForm.yueduContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.yueduContent = this.ruleForm.yueduContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`yuedu/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.yueduCrossAddOrUpdateFlag = false;
                                        this.parent.search();
